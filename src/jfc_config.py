...
# Safely import standard and external dependencies
...
FALLBACK_JIOFIBER_IP = "192.168.31.1"
...
def setup_logger(log_level: int):
    ...
    # 2. Access Log Handler (Only INFO and DEBUG)
    ...
    # 3. Error Log Handler (WARNING, ERROR, CRITICAL)
...
def generate_asterisk_config(extracted_values: dict, jiofiber_hostname: str):
    """Generates an Asterisk pjsip.conf configuration for Linux/Termux setups."""
    domain = extracted_values.get('home_network_domain_name', 'xx.wln.ims.jio.com')
    user = extracted_values.get('username', 'UNKNOWN')
    pwd = extracted_values.get('userpwd', '')

    proxy = f"sip:{jiofiber_hostname}:5068\\;lr"

    asterisk_config = f"""
; ==========================================================
; Asterisk PJSIP Configuration for JioFiber
; Auto-generated by RatioJuris
; ==========================================================
...
def write_configuration_files(extracted_values: dict, jiofiber_hostname: str):
    """Handles parsing extracted values and writing configs out to the system."""
    ...
    generate_asterisk_config(extracted_values, jiofiber_hostname)
...
def parse_sip_config(sip_config: ET.Element, jiofiber_hostname: str):
    params_to_extract = [
    ...
    save_cache(jiofiber_hostname, extracted_values)
    write_configuration_files(extracted_values, jiofiber_hostname)


def main(no_otp: bool, is_cron: bool, force_refresh: bool):
    cache = load_cache()
    
    # 1. Attempt to use cached configurations directly if not performing a forced renewal
    if not force_refresh and "extracted_values" in cache and "jiofiber_domain" in cache:
        logging.info("[*] Valid cache signature found. Skipping network processing requests.")
        write_configuration_files(cache["extracted_values"], cache["jiofiber_domain"])
        return

    # 2. Extract domain information safely (incorporating Fallback IP Logic)
    jiofiber_domain = None
    if not force_refresh and cache.get("jiofiber_domain"):
        jiofiber_domain = check_domain(cache.get("jiofiber_domain"))

    if jiofiber_domain is None:
        jiofiber_domain = check_domain(DEFAULT_JIOFIBER_HOSTNAME)
        
        # 3. Fallback IP sequence triggered if default domain fails
        if jiofiber_domain is None:
            logging.info(f"Default hostname '{DEFAULT_JIOFIBER_HOSTNAME}' failed. Trying fallback IP: {FALLBACK_JIOFIBER_IP}...")
            jiofiber_domain = check_domain(FALLBACK_JIOFIBER_IP)

    # 4. Final resort manual input if automated discovery entirely fails
    while jiofiber_domain is None:
        if is_cron:
            logging.error("Cron mode active: Cannot interactively prompt for missing domain. Exiting.")
            sys.exit(1)
            
        logging.info("Couldn't find JioFiber domain/IP automatically!")
        input_domain = input("Please enter the JioFiber domain/IP manually: ")
    ...
    while try_config_count < 3:
        ...
        if sip_configuration_response.status_code != 200:
            ...
            logging.info(f"Hostname: {hostname} isn't registered on JioFiber SIP yet!")
            
            if not ims_register(jiofiber_domain, DEFAULT_JIOFIBER_HTTPS_PORT, hostname, mac, is_cron):
...
if __name__ == "__main__":
    ...
    if not args.cron:
        ...
        print("=" * 80)
        print("                                RatioJuris                                ")
        print("            (JioFiber SIP Configuration Tool for Multi-Platform)          ")
        print("=" * 80)
    ...
    # Auto-close upon success. Code completes execution implicitly here.
